    ...
    return (
        <AdminLayout title="Hero Carousel">
            ...
            <div className="flex items-center justify-between mb-7">
                <div><h1 className="text-xl font-bold text-white">Hero Carousel</h1><p className="text-gray-500 text-sm mt-0.5">Manage the homepage hero slides</p></div>
            ...
            <div className="space-y-4">
                {slides.map((slide) => (
                    <div key={slide.id} className="bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col sm:flex-row">
                        ...
                        <div className="flex-1 p-5 flex items-center justify-between gap-4">
                            <div>
                                ...
                                <p className="text-gray-500 text-xs">Order: {slide.sort_order} · CTA: {slide.cta_text}</p>
                            </div>
                            <div className="flex gap-2 shrink-0">
                                <Link href={`/admin/hero/${slide.id}/edit`} className="text-gray-600 hover:text-white bg-white/5 hover:bg-gray-100 text-xs px-3 py-2 rounded-lg transition-all">Edit</Link>
                ...
                {slides.length === 0 && <p className="text-center text-gray-500 py-16">No slides yet. Add your first hero slide.</p>}
            </div>
        </AdminLayout>
    );
}
