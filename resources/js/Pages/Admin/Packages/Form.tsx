        ...
        <AdminLayout title={pkg ? 'Edit Package' : 'New Package'}>
            ...
            <div className="max-w-2xl">
                ...
                <form onSubmit={submit} className="bg-white border border-gray-200 rounded-2xl p-7 space-y-5">
                    <div>
                        <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Package Name *</label>
                        <input type="text" value={data.name} onChange={e => setData('name', e.target.value)} required className={inputCls} />
                        {errors.name && <p className="text-red-400 text-xs mt-1">{errors.name}</p>}
                    </div>
                    <div>
                        <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Tagline</label>
                    ...
                    {/* Image Upload */}
                    <div>
                        <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Package Image</label>
                        {preview && (
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div>
                            <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Price *</label>
                        ...
                        <div>
                            <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Price Note</label>
                        ...
                        <div>
                            <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Duration *</label>
                    ...
                    <div>
                        <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Badge</label>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className="text-gray-600 text-xs uppercase tracking-wider">What's Included *</label>
                            <button type="button" onClick={addFeature} className="text-[#d4a853] text-xs hover:underline">+ Add item</button>
                        </div>
                        {data.features.map((f, i) => (
                            <div key={i} className="flex gap-2 mb-2">
                                <input type="text" value={f} onChange={e => updateFeature(i, e.target.value)} required placeholder={`Item ${i + 1}`} className="flex-1 bg-white/5 border border-gray-200 focus:border-[#d4a853]/60 text-white rounded-xl px-4 py-2.5 text-sm outline-none" />
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="flex items-center gap-2 pt-7">
                            ...
                            <label htmlFor="feat" className="text-gray-600 text-sm">Featured</label>
                        </div>
                        <div className="flex items-center gap-2 pt-7">
                            ...
                            <label htmlFor="act" className="text-gray-600 text-sm">Active</label>
                        </div>
                    </div>
                    <div className="flex gap-3 pt-2">
                        ...
                        <a href="/admin/packages" className="px-5 py-3 border border-gray-200 text-gray-600 hover:text-white text-sm rounded-xl text-center">Cancel</a>
                    </div>
                </form>
            </div>
        </AdminLayout>
