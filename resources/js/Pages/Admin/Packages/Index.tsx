    ...
    return (
        <AdminLayout title="Tour Packages">
            ...
            <div className="flex items-center justify-between mb-7">
                <div><h1 className="text-xl font-bold text-white">Tour Packages</h1><p className="text-white/40 text-sm mt-0.5">{packages.length} packages</p></div>
            ...
            <div className="space-y-4">
                {packages.map((pkg) => (
                    <div key={pkg.id} className="bg-[#071510] border border-white/8 rounded-2xl flex flex-col sm:flex-row overflow-hidden">
                        ...
                        <div className="flex-1 p-5 flex items-center justify-between gap-4">
                            ...
                            <div className="flex gap-2 shrink-0">
                                <Link href={`/admin/packages/${pkg.id}/edit`} className="text-white/50 hover:text-white bg-white/5 hover:bg-white/10 text-xs px-3 py-2 rounded-lg">Edit</Link>
                ...
                {packages.length === 0 && <p className="text-center text-white/40 py-16">No packages yet.</p>}
            </div>
        </AdminLayout>
    );
}
