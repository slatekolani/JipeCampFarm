    ...
    return (
        <AdminLayout title="About Section">
            ...
            <div className="max-w-2xl">
                ...
                <form onSubmit={submit} className="bg-white border border-gray-200 rounded-2xl p-7 space-y-6">
                    <div>
                        <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Heading *</label>
                    ...
                    <div>
                        <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Subheading</label>
                        <input type="text" value={data.subheading} onChange={e => setData('subheading', e.target.value)} className={inputCls} />
                    </div>
                    {/* About Image Upload */}
                    <div>
                        <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">About Image</label>
                        {preview && (
                        ...
                        <div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-gray-200 hover:border-[#d4a853]/50 rounded-xl p-6 text-center cursor-pointer transition-colors">
                            ...
                            <p className="text-gray-500 text-sm">{data.image ? data.image.name : 'Click to upload image'}</p>
                            <p className="text-gray-300 text-xs mt-1">JPG, PNG, WebP — max 8MB</p>
                        </div>
                        <input ref={fileRef} type="file" accept="image/*" onChange={handleFile} className="hidden" />
                    </div>
                    <div>
                        <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Main Body *</label>
                    ...
                    <div>
                        <label className="block text-gray-600 text-xs uppercase tracking-wider mb-1.5">Secondary Body</label>
                    ...
                    {/* Stats */}
                    <div>
                        <label className="block text-gray-600 text-xs uppercase tracking-wider mb-3">Key Stats</label>
                        <div className="grid grid-cols-2 gap-3">
                            {data.stats.map((s, i) => (
                                <div key={i} className="bg-white/3 border border-gray-200 rounded-xl p-3">
                                    <input type="text" placeholder="Value (e.g. 6+)" value={s?.value ?? ''} onChange={e => updateStat(i, 'value', e.target.value)} className="w-full bg-transparent text-white text-sm outline-none placeholder-gray-400 mb-1" />
                                    <input type="text" placeholder="Label (e.g. Activities)" value={s?.label ?? ''} onChange={e => updateStat(i, 'label', e.target.value)} className="w-full bg-transparent text-gray-500 text-sm outline-none placeholder-gray-400" />
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-3">
                            <label className="text-gray-600 text-xs uppercase tracking-wider">Key Features List</label>
                            <button type="button" onClick={addFeature} className="text-[#d4a853] text-xs hover:underline">+ Add</button>
                        </div>
                        {data.features.map((f, i) => (
                            <div key={i} className="flex gap-2 mb-2">
                                <input type="text" value={f} onChange={e => updateFeature(i, e.target.value)} placeholder={`Feature ${i + 1}`} className="flex-1 bg-white/5 border border-gray-200 focus:border-[#d4a853]/60 text-white rounded-xl px-4 py-2.5 text-sm outline-none" />
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <label htmlFor="active" className="text-gray-600 text-sm">Active</label>
                    </div>
